<template>
  <div class="w-full flex items-center justify-center flex-col pt-14">
    ...
    <h1 class="card-title mt-4">{{ source.CurrentTrack ?? 'Play Something' }}</h1>
      <p class="line-clamp-1 px-8 w-full text-center">{{ source.CurrentArtist ?? 'Artists' }} | {{ source.CurrentAlbum ?? 'Album' }}</p>

    <div class="w-full flex items-center justify-between mt-10">

      <button @click="source.audio.loop = !source.audio.loop" :class="source.audio.loop ? 'text-slate-300':'text-slate-500'">
